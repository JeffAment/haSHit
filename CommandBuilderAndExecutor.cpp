...
//todo: add progress bar to bottom of shell
...
bool CommandBuilderAndExecutor::md5FileExists(const bfs::directory_entry &de) {
    ...
    std::ifstream md5File(filePathWithMd5Appendage);
    if (md5File.good()) {
        md5File.close();
...
bool CommandBuilderAndExecutor::isMd5File(const bfs::directory_entry &de) const {
    std::string filePath = de.path().string();
    std::string lastFourCharacters = filePath.substr(filePath.length() - 4);
    return lastFourCharacters == ".md5";
}
...
void CommandBuilderAndExecutor::printGenerateStatusReport() const {
    ...
    std::cout << md5FilesThatCouldntBeCreated << " .md5 files couldn't be generated (possible access error; check permissions)" << std::endl;
